...
class Operator(OperatorBase):
    ...
    def original_save_scan(self, filename, metadata=None, store_conf=False):
        """
    ...
    def save_scan(self, filename, store_conf=False):
        """
        ...
        :type store_conf: bool
        """
        # First test if thфe required data arrays have been generated (i.e. if the scan has run)
        if not hasattr(self, "scan_voltages") or not hasattr(self, "measured_voltages"):
            self.logger.warning('no data to save yet')
            return
        if os.path.exists(filename):
            self.logger.warning('overwriting existing file: {}'.format(filename))
        self.logger.debug('Saving data')
        data = [[self.scan_voltages[i], self.measured_voltages[i]] for i in range(0, len(self.scan_voltages))]
