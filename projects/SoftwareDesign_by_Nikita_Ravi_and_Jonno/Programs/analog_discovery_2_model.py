...
class Operator(OperatorBase):
    ...
    def _verify_scan_channels_modified(self):
        """
        Checks if channels in properties are valid and returns analog out and analog in channel.
        Note that None is returned if an error is found
        ...
        if 'scan' not in self.properties:
            self.logger.error("'scan' not found in properties")
            return
        if 'ai_channel1' not in self.properties['scan'] or self.properties['scan']['ai_channel1'] not in [1,2]:
            self.logger.error("'ai_channel1' not found in properties or invalid value (should be 1 or 2)")
            return
        if 'ai_channel2' not in self.properties['scan'] or self.properties['scan']['ai_channel2'] not in [1,2]:
            self.logger.error("'ai_channel2' not found in properties or invalid value (should be 1 or 2)")
            return
        return self.properties['scan']['ai_channel1'], self.properties['scan']['ai_channel2']
    ...
    def _set_scan_start_modified(self, value):
        """
        Set scan start value.
        Forces value to be in valid range and updates properties dictionary.
        Also corrects the sign of step if required.
        ...
        :type value: float
        """
        ai_ch1, ai_ch2 = self._verify_scan_channels()
        if ai_ch1 is None:  # if _verify_scan_channels() returns nothing that means channel is invalid or not found
            return
        if (value <= 0):
            self.logger.error("Start time can't be smaller than 0")
        else:
            self.properties['scan']['start'] = value
        self._set_scan_step()
    ...
    def _set_scan_stop_modified(self, value):
        """
        Set scan stop value.
        Forces value to be in valid range and updates properties dictionary.
        ...
        if ai_ch1 is None:  # if _verify_scan_channels() returns nothing that means channel is invalid or not found
            return
        if (value >= 1000000):
            self.logger.error("Measuring time can't be greater than 1000000 just because computer won't be able to handle it")
        if (value < 0):
            self.logger.error("Back in time? Not today")
        else:
            self.properties['scan']['stop'] = value
        self._set_scan_step()

    def _set_scan_duration(self, value):
        """
         Set scan duration value.
         Forces value to be in valid range and updates properties dictionary.

         :param value: duration value of scan (S)
         :type value: float
        """
        ai_ch1, ai_ch2 = self._verify_scan_channels()
        if ai_ch1 is None:  # if _verify_scan_channels() returns nothing that means channel is invalid or not found
        ...
        if (value >= 1000000):
            self.logger.error(
                "Measuring time can't be greater than 1000000 just because computer won't be able to handle it")
        if (value < 0):
            self.logger.error("Back in time? Not today")
        else:
            self.properties['scan']['duration'] = value
        self._set_scan_step()
    ...
    def _set_scan_step_modified(self, step=None):
        """
        Set scan step value. Note that it will correct the sign automatically accroding to start and stop values.
        If no step value is supplied it only corrects the sign of step.
    ...
    def do_scan_modified(self, param=None):
        """
        An example of a method that performs a scan (based on parameters in the config file).
        This method can be run from a GUI, from command line or other script
        This scan sweeps the voltage on one of the AO channels and reads one of the AI channels.
        ...
        if 'scan' not in self.properties:
            self.logger.error("The config file or properties dict should contain 'scan' section.")
            return
        required_keys = ['duration', 'step', 'ai_channel']
        if not all(key in self.properties['scan'] for key in required_keys):
            self.logger.error("'scan' should contain: " + ', '.join(required_keys))
            return
        try:
            duration = self.properties['scan']['duration']
            step = self.properties['scan']['step']
            ch_ai1 = int(self.properties['scan']['ai_channel1'])
            ch_ai2 = int(self.properties['scan']['ai_channel2'])
        except:
            self.logger.error("Error occured while reading scan config values")
            return
        if ch_ai1 not in [1, 2]:
            self.logger.error("AI channel 1 need to be 1 or 2")
            return
        if ch_ai2 not in [1, 2]:
            self.logger.error("AI channel 2 need to be 1 or 2")
            return
        if 'stabilize_time' in self.properties['scan']:
            stabilize = self.properties['scan']['stabilize_time']
        else:
            self.logger.info("stabilize_time not found in config, using 0s")
            stabilize = 0
        num_points = np.int(round((duration) / step + 1))  # use round to catch the occasional rounding error
        if num_points <= 0:
            self.logger.error("Start, stop and step result in 0 or fewer points to sweep")
            return

        self.time_stamps_to_scan = np.linspace(0, duration, num_points)

        self.ch1_scan_results = []
        self.ch2_scan_results = []
        self.current_time_scale = []
        self._busy = True  # indicate that operator is busy

        for i, time_stamp in enumerate(self.time_stamps_to_scan):
            self.logger.debug('applying {} on channel {} and channel {}'.format(time_stamp, ch_ai1,ch_ai2))
            sleep(stabilize)
            voltage_from_channels = self.analog_in()
            measured1 = voltage_from_channels[0]
            measured2 = voltage_from_channels[1]
            self.current_time_scale.append(time_stamp)
            self.ch1_scan_results.append(measured1)
            self.ch2_scan_results.append(measured2)
        ...
        return self.ch1_scan_results, self.ch2_scan_results

    def scan_two_input_votlages(self, param=None):
        """
        An example of a method that performs a scan (based on parameters in the config file).
        This method can be run from a GUI, from command line or other script
        This scan sweeps the voltage on one of the AO channels and reads one of the AI channels.
        ...
        if 'scan' not in self.properties:
            self.logger.error("The config file or properties dict should contain 'scan' section.")
            return
        required_keys = ['start', 'stop', 'step', 'ai_channel']
        if not all(key in self.properties['scan'] for key in required_keys):
            self.logger.error("'scan' should contain: "+', '.join(required_keys))
            return
        try:
    ...
    def save_scan_modified(self, filename, store_conf=False):
        """
    Store data in array and save to csv file.
    Optionally stores the entire Operator properties dictionary to a yaml file of the same name.
    ...
    """
    # First test if the required data arrays have been generated (i.e. if the scan has run)
        if not hasattr(self, "ch1_scan_results") or not hasattr(self, "ch2_scan_results") or not hasattr(self,"current_time_scale"):
            self.logger.warning('no data to save yet')
            return
        if os.path.exists(filename):
            self.logger.warning('overwriting existing file: {}'.format(filename))
        self.logger.debug('Saving data')
        data = [[self.current_time_scale[i], self.ch1_scan_results[i], self.ch2_scan_results[i]] for i in range(0, len(self.current_time_scale))]
